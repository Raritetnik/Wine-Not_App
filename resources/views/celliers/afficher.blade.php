...
@section('content')
    ...
    <div class="conteneur-de-toute-la-page">
        <div class="container mx-auto pb-8 pt-2 sm:max-w-xl md:max-w-full lg:max-w-screen-xl md:px-24 lg:px-8 lg:py-20">
            <div>
                <div class="bg-gray-100 flex gap-1 border px-4 py-3 rounded-md justify-between w-full max-w-[560px] mx-auto"
        ...
        <div id="popup-modal" tabindex="-1"
            class="bg-black bg-opacity-40 fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full ">
            <div
                    ...
                    <div class="p-6 text-center">
                        ...
                        <form action="{{ route('supprimer.bouteille') }}" method="post" enctype="multipart/form-data">
                            ...
                            <button type="submit" data-modal-hide="popup-modal"
                                class="p-2 px-4 bg-green-600 text-white rounded" type="button" alt="delete">Oui</button>
                            <button data-modal-hide="popup-modal" class="p-2 px-4 bg-gray-800 text-white rounded"
                                type="button" id="no_popup-modal">No</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    <v-filtre :type="{{ $type }}" :pays="{{ $pays }}" :cellier="{{ $cellier }}"
        :bouteilles="{{ $bouteilles }}" :liste="{{ $liste }}" />
@endsection
