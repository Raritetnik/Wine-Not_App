  ...
  <form method="post" enctype="multipart/form-data" class="w-full pt-3">
    ...
    <div class="mb-2 flex justify-between gap-3">
      <div class="w-1/2 mb-3">
        <label for="qty" class="block text-section_title font-bold mb-2">Quantité</label>
        <input class="block w-full py-3 px-3 placeholder-section_title rounded-md border border-gray-300 focus:border-secondary focus:outline-none appearance-none {{ $errors->has('qty') ? 'border-error' : '' }}" name="qty" id="qty" min="1" placeholder="Quantité" value="{{ old('qty') }}" required>
        </input>
    ...
    <div class="mt-7 flex">
      <button type="submit" class="w-1/2 px-7 py-1.5 rounded-md bg-secondary text-lg font-medium text-main hover:bg-transparent border hover:border-secondary hover:text-secondary transition duration-200 ease-in-out">Ajouter</button>
    </div>
